...
// TOGGLE DETAILS FOR SOCIAL MEDIA ACCORDIONS
function toggleSocialDetails(containerId, btn) {
    ...
    if (isShown) {
        ...
        btn.innerHTML = `<span>Show Details</span> <i class="fa-solid fa-chevron-down"></i>`;
    } else {
        targetContainer.classList.add('show');
        btn.setAttribute('aria-expanded', 'true');
        btn.innerHTML = `<span>Hide Details</span> <i class="fa-solid fa-chevron-up"></i>`;
    }
}
    ...
    el.innerHTML = items.map(item => `
        <article class="card">
            ...
            <div class="card-body">
                ...
                <a href="${item.link}" target="_blank" rel="noopener" class="btn btn-primary btn-block">
                    <span>Get Deal Offer</span> <i class="fa-solid fa-arrow-up-right-from-square"></i>
                </a>
            </div>
        </article>
    `).join('');
    ...
    el.innerHTML = items.map(item => `
        <article class="card">
            ...
            <div class="card-body">
                ...
                <h3 class="card-title">${item.title}</h3>
                <p class="card-text">${item.description}</p>
                <a href="${item.link}" class="btn btn-secondary btn-block">
                    <span>Read Guide</span> <i class="fa-solid fa-book-open"></i>
                </a>
            </div>
        </article>
    `).join('');
    ...
    el.innerHTML = items.map(item => `
        <article class="card">
            ...
            <div class="card-body">
                ...
                <h3 class="card-title">${item.title}</h3>
                <p class="card-text">${item.description}</p>
                <a href="${item.link}" class="btn btn-outline btn-block">
                    <span>Read Insight</span> <i class="fa-solid fa-arrow-right"></i>
                </a>
            </div>
        </article>
    `).join('');
...
function initFilters() {
    ...
    if (dealsSearch) {
        dealsSearch.addEventListener('input', (e) => {
            ...
            const filtered = APP_DATA.deals.filter(d => d.title.toLowerCase().includes(query) || d.category.toLowerCase().includes(query));
            renderDeals('deals-page-grid', filtered);
        });
    }

    const guidesSearch = document.getElementById('guides-search');
    if (guidesSearch) {
        guidesSearch.addEventListener('input', (e) => {
            const query = e.target.value.toLowerCase();
            const filtered = APP_DATA.guides.filter(g => g.title.toLowerCase().includes(query) || g.category.toLowerCase().includes(query));
            renderGuides('guides-page-grid', filtered);
        });
    }
}
    ...
    form.addEventListener('submit', () => {
        ...
        if (val) {
            ...
            setTimeout(() => {
                const dealsSearch = document.getElementById('deals-search');
                if (dealsSearch) {
                    dealsSearch.value = val;
                    dealsSearch.dispatchEvent(new Event('input'));
                }
            }, 100);
        }
    });
